...
namespace IAEngine
{
	...
	void FreeCamera::Update(InputManager* pInputManager, float fElaspedTime)
	{
		...
		//update angle
		m_fAngleY += pInputManager->GetMouseMoveX() * fMouseSensibility;
		m_fAngleX += -pInputManager->GetMouseMoveY() * fMouseSensibility;

		m_vDirection.x = sin(m_fAngleY) * cos(m_fAngleX);
		m_vDirection.y = sin(m_fAngleX);
		m_vDirection.z = cos(m_fAngleY) * cos(m_fAngleX);
		m_vDirection.Normalize();
		//update 
		Vector3 vRight;
		m_vUp.Cross(m_vDirection, vRight);
